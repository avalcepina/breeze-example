...
object MatrixOpsHelper {
  ...
  private def filterValue(x: Double, range: Range) = if (x <= range.start || x >= range.end) 0.0 else x
  ...
  /**
    ...
    * @return
    */
  def validateMatrix(matrix: CSCMatrix[Double], range: Range) = !matrix.flatten(View.Prefer).exists(isInvalidValue(_, range))
  ...
  def matrixAverage(matrix: CSCMatrix[Double]) = {
    ...
    matrix.pairs.foreach(v => {
      result(v._1._1, v._1._2) = calculateSum(v._2, sliceMatrix(matrix, v._1._1, v._1._2).flatten(View.Prefer))
    })
